  ...
  _addAPIEventHandlers() {
    ...
    const handlerWithDispatch = (event, ...args) => {
      ...
      if (item) {
        ...
        this.dispatch('conversation.updated', { item, delta, response_id: event.response_id });
      }
      return { item, delta };
    };
    const callTool = async (tool, itemId, responseId) => {
      try {
        ...
        const result = await toolConfig.handler(jsonArguments, itemId, responseId);
    ...
    this.realtime.on('server.response.output_item.done', async (event) => {
      ...
      // If a relay server is used, the tools gets called from server. In server the handlers are not present, hence
      // it adds a message saying function not found
      // This is just a guard against that
      // Issue details: https://github.com/openai/openai-realtime-api-beta/issues/14
      const tool = item.formatted.tool
      if (tool && this.tools[tool.name]?.handler) {
        callTool(tool, item.id, event.response_id);
      }
    });
